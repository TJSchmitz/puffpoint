...
export const setRole = functions.https.onCall(async (req) => {
  if (!req.auth) throw new functions.https.HttpsError('unauthenticated', 'Auth required');
  ...
  const { uid, role, value } = (req.data ?? {}) as { uid: string; role: 'admin' | 'mod'; value: boolean };
...
export const nominatimSearch = functions.https.onCall(async (req) => {
  const q = (req.data?.q as string | undefined)?.trim();
...
export const moderatePhoto = functions.https.onCall(async (req) => {
  ...
  const { photoId, action } = (req.data ?? {}) as { photoId: string; action: 'approve' | 'reject' };
...
export const createInvite = functions.https.onCall(async (req) => {
  if (!req.auth) throw new functions.https.HttpsError('unauthenticated', 'Auth required');
  const { listId, role } = (req.data ?? {}) as { listId: string; role: 'view' | 'edit' };
  ...
  if (!listSnap.exists) throw new functions.https.HttpsError('not-found', 'List not found');
  ...
  const count = (counterSnap.exists ? (counterSnap.data()!.count as number) : 0) + 1;
  if (count > 20) throw new functions.https.HttpsError('resource-exhausted', 'Invite limit reached');
...
export const redeemInvite = functions.https.onCall(async (req) => {
  ...
  if (inviteSnap.empty) throw new functions.https.HttpsError('not-found', 'Invite not found');
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!listSnap.exists) throw new functions.https.HttpsError('not-found', 'List missing');
...
export const listUsers = functions.https.onCall(async (req) => {
  ...
  const pageToken = (req.data?.pageToken as string | undefined) ?? undefined;
